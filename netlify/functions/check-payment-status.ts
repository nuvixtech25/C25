...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  try {
    ...
    if (!orderError && orderData) {
      ...
      console.log(
        "Nenhum resultado encontrado na tabela orders, verificando tabela asaas_payments...",
      );
    }
    ...
    if (!paymentError && paymentData) {
      console.log(
        `Status encontrado na tabela asaas_payments: ${paymentData.status}`,
      );
    ...
    try {
      ...
      if (!apiKey) {
        console.error(
          `Chave de API ${usesSandbox ? "sandbox" : "produção"} não configurada`,
        );
        throw new Error(
          `Chave de API ${usesSandbox ? "sandbox" : "produção"} não configurada`,
        );
      ...
      if (!response.ok) {
        ...
        console.error(
          `Erro na API do Asaas: ${response.status} - ${errorText}`,
        );
    ...
    console.log(
      "Nenhum dado de pagamento encontrado, retornando status padrão PENDING",
    );
    return {
      statusCode: 200,
      headers,
