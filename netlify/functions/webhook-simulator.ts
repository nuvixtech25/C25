...
async function sendAdminNotification(payment: any, orderData: any) {
  try {
    ...
    try {
      ...
      if (!emailResponse.ok) {
        console.error(
          "Erro ao enviar notificação por email (simulação):",
          await emailResponse.text(),
        );
      } else {
        console.log(
          "Notificação de pagamento simulado enviada com sucesso para",
          ADMIN_EMAIL,
        );
      }
    } catch (error) {
  ...
  try {
    ...
    if (payload.event && payload.payment) {
      ...
      const isManualCardPayment =
        payload.payment.id === "manual_card_payment" && payload.orderId;

      // Log payload details for debugging
      if (isManualCardPayment) {
        console.log(
          `[AUDIT] Processing manual card webhook for order ${payload.orderId} with event ${payload.event}`,
        );
      } else {
        console.log(
          `[AUDIT] Processing webhook for payment ${payload.payment.id} with event ${payload.event} and status ${payload.payment.status}`,
        );
      }
      ...
      if (!isManualCardPayment) {
        ...
        if (paymentsError) {
          console.log(
            "[AUDIT] Note: Could not update asaas_payments table:",
      ...
      await supabase.from("asaas_webhook_logs").insert({
        ...
        payment_id: isManualCardPayment
          ? `manual_${payload.orderId}`
          : payload.payment.id,
        status: newStatus,
        payload: payload,
      });
...
⏰ <b>Data:</b> ${new Date().toLocaleString("pt-BR")}`;
          ...
          console.error(
            "[AUDIT] Error sending Telegram notification:",
            notificationError,
          );
        }
      }
