...
export function validateAsaasCustomerRequest(
  ...
  try {
    ...
    if (typeof data.value === "string") {
      ...
      if (isNaN(normalizedValue)) {
        console.error(
          `Erro ao converter valor para número: "${data.value}" -> "${cleanedValue}" -> NaN`,
        );
        throw new Error(
          `Valor inválido: "${data.value}" não é um número válido`,
        );
      }
    ...
    if (normalizedValue <= 0) {
      throw new Error(
        `Valor deve ser maior que zero, valor recebido: ${normalizedValue}`,
      );
    }
    ...
    throw new Error(
      `Dados inválidos: ${error.message || "formato JSON inválido"}`,
    );
  }
}
