...
const RetryPaymentButton: React.FC<RetryPaymentButtonProps> = ({
  order,
  isLoading,
}) => {
  ...
  const handleRetry = () => {
    ...
    if (!validation.valid) {
      logPaymentError("RetryPaymentButton", validation.message, {
        orderId: order?.id,
      });
      toast({
        title: "Erro",
    ...
    console.log(
      "[RetryPaymentButton] Navigating to retry-payment with order ID:",
      order!.id,
    );
    ...
    const safeOrderData = {
      ...
      has_whatsapp_support:
        typeof order!.has_whatsapp_support === "boolean"
          ? order!.has_whatsapp_support
          : false,
      whatsapp_number:
        typeof order!.whatsapp_number === "string"
    ...
    console.log(
      "[RetryPaymentButton] Complete order object for retry:",
      safeOrderData,
    );
