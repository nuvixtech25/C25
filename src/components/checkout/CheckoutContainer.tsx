...
const CheckoutContainer: React.FC<CheckoutContainerProps> = ({
  ...
  const [dbCustomization, setDbCustomization] =
    useState<CheckoutCustomizationDB>({
      button_color: "#28A745",
      button_text_color: "#ffffff",
      button_text: "Finalizar Pagamento",
      header_message: "Oferta por tempo limitado!",
      banner_image_url:
        "/lovable-uploads/0316cf52-2668-4bd5-8293-a0b8e5f3b8fb.png",
      show_banner: true,
    ...
    const fetchCustomization = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .select(
            "button_color, button_text_color, button_text, header_message, banner_image_url, show_banner, heading_color, banner_color",
          )
        ...
        if (error) {
          ...
          toast({
            ...
            description:
              "Erro ao carregar as configurações do checkout. Verifique o console para mais detalhes.",
            variant: "destructive",
          });
          return;
        }
  ...
  const useProductCustomColors =
    customization && customization.useGlobalColors === false;

  // Use properly typed CSS variables with our custom interface
  const customStyles: CustomCSSProperties = {
    "--button-color":
      useProductCustomColors && customization?.buttonColor
    ...
    "--heading-color":
      useProductCustomColors && customization?.headingColor
        ? customization.headingColor
        : dbCustomization.heading_color || "#000000",
    "--banner-color":
      useProductCustomColors && customization?.bannerColor
  ...
  return (
    <div
      className="flex flex-col bg-white text-black max-w-full overflow-x-hidden"
        ...
        <div className="w-full mx-auto bg-white">
          <div>
            {dbCustomization.show_banner && (
              <CountdownBanner
                message={
                  dbCustomization.header_message || "Oferta por tempo limitado!"
                }
