...
export const handleCepChange = (
  ...
  const specialCharsBefore = (value.substring(0, cursorPos).match(/[-]/g) || [])
    .length;
  ...
  const specialCharsAfter = (
    formatted.substring(0, newCursorPos).match(/[-]/g) || []
  ).length;
...
export const handlePhoneChange = (
  ...
  // Count special chars before cursor
  const specialCharsBefore = (
    value.substring(0, cursorPos).match(/[()-\s]/g) || []
  ).length;
  ...
  // Adjust for deleted characters
  const specialCharsAfter = (
    formatted.substring(0, newCursorPos).match(/[()-\s]/g) || []
  ).length;
  newCursorPos -= Math.max(0, specialCharsBefore - specialCharsAfter);
