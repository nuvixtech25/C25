...
export async function sendTelegramNotification(
  ...
  try {
    ...
    if (filteredBots.length === 0) {
      console.warn(
        `No bots configured to receive ${notificationType} notifications`,
      );
      return false;
    }
    ...
    const sendPromises = filteredBots.map(async (bot: any) => {
      try {
        ...
        if (!response.ok) {
          ...
          console.error(
            `Failed to send Telegram notification to bot ${bot.name}:`,
            errorBody,
          );
          return false;
        }
        ...
        console.log(
          `Telegram notification sent successfully to bot ${bot.name}`,
        );
        return true;
      } catch (error) {
        console.error(
          `Error sending Telegram notification to bot ${bot.name}:`,
          error,
        );
        return false;
      }
