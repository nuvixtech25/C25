...
describe("usePaymentPolling", () => {
  ...
  it("should create polling interval for regular payment IDs", () => {
    const mockCheckOrderStatusInDatabase = vi
      .fn()
      .mockResolvedValue({ statusChanged: false });
    const mockCheckPaymentStatusInAsaas = vi
      .fn()
      .mockResolvedValue({ statusChanged: false });
    ...
    expect(mockCheckOrderStatusInDatabase).toHaveBeenCalledWith(
      mockOrder,
      mockFetchOrderById,
    );
