...
export const usePaymentStatusChecker = () => {
  ...
  const hasTerminalStatus = (
    order: Order,
  ): { hasTerminalStatus: boolean; isSuccess: boolean } => {
    // Check for failure statuses
    if (
      order.status &&
      FAILURE_STATUSES.includes(order.status as PaymentStatus)
    ) {
    ...
    if (
      order.status &&
      SUCCESS_STATUSES.includes(order.status as PaymentStatus)
    ) {
  ...
  const checkOrderStatusInDatabase = async (
    ...
    try {
      ...
      if (!currentOrder.id) {
        console.log(
          "[PaymentAnalysis] Order ID is undefined, skipping database check",
        );
      ...
      if (FAILURE_STATUSES.includes(refreshedOrder.status as PaymentStatus)) {
        console.log(
          "[PaymentAnalysis] Order status updated to failed in database",
        );
        return {
          statusChanged: true,
      ...
      console.error(
        "[PaymentAnalysis] Error checking order status in database:",
        err,
      );
      return { statusChanged: false };
    }
  ...
  const checkPaymentStatusInAsaas = async (
    ...
    if (
      ...
      console.log(
        "[PaymentAnalysis] Using temporary ID, waiting for backend update: " +
          currentOrder.asaasPaymentId,
      );
      return { statusChanged: false };
    }
    ...
    console.log(
      `[PaymentAnalysis] Payment status check #${checkCount}:`,
      status,
    );
    ...
    if (
      ...
      SUCCESS_STATUSES.includes(status as PaymentStatus)
    ) {
    ...
    if (
      typeof status === "string" &&
      FAILURE_STATUSES.includes(status as PaymentStatus)
    ) {
      return { statusChanged: true, status: status as PaymentStatus };
    }
