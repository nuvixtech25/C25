...
export const usePaymentPolling = ({
  ...
  const { checkOrderStatusInDatabase, checkPaymentStatusInAsaas } =
   ...
   */
  const handleTemporaryPaymentId = (
    order: Order,
    ...
    if (
      ...
      console.log(
        "[PaymentAnalysis] Using temporary ID, proceeding to success after delay",
      );
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (newCount >= maxChecks) {
          ...
          console.log(
            "[PaymentAnalysis] Max checks reached, navigating to success",
          );
          navigateToSuccess(order);
          return;
        }
        ...
        const dbResult = await checkOrderStatusInDatabase(
          order,
          fetchOrderById,
        );
        ...
        if (asaasResult.statusChanged) {
          ...
          if (
            asaasResult.status &&
            ["CONFIRMED"].includes(asaasResult.status)
          ) {
            navigateToSuccess(order);
          } else {
        ...
        console.error(
          "[PaymentAnalysis] Error checking payment status:",
          error,
        );
      }
    }, pollingInterval);
