...
export const usePixPaymentStatus = ({
  ...
  const checkStatus = async () => {
    if (!paymentId || !orderId) {
      console.error(
        "Não é possível verificar o status sem paymentId ou orderId",
      );
      return;
    }
    ...
    try {
      console.log(
        `Checking payment status for paymentId: ${paymentId}, orderId: ${orderId}`,
      );
      ...
      if (typeof response === "object" && response !== null) {
        ...
        console.log(
          `Received payment status: ${response.status}, normalized to: ${normalizedStatus}`,
        );
        ...
        console.log(
          `Received payment status: ${response}, normalized to: ${normalizedStatus}`,
        );
        setStatus(normalizedStatus as PaymentStatus);
