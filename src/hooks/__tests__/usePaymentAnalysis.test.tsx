...
vi.mock("@/hooks/payment/usePaymentStatusChecker", () => ({
  usePaymentStatusChecker: () => ({
    hasTerminalStatus: vi
      .fn()
      .mockReturnValue({ hasTerminalStatus: false, isSuccess: false }),
  ...
  it("should handle errors during order loading", async () => {
    ...
    vi.mock("@/hooks/useOrderData", () => ({
      useOrderData: () => ({
        fetchOrderById: vi
          .fn()
          .mockRejectedValue(new Error("Failed to load order")),
        getOrderIdFromUrl: vi.fn().mockReturnValue("order-123"),
      }),
    }));
    ...
    const { result, rerender } = renderHook(() => usePaymentAnalysis(), {
      wrapper,
    });
