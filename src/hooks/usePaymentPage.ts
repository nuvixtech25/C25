...
export const usePaymentPage = () => {
  ...
  const { paymentStatus, isCheckingStatus, refreshStatus } =
    usePixStatusTracker(paymentData, order);
  ...
  useEffect(() => {
    ...
    if (!billingData || !orderData) {
      ...
      toast({
        title: "Erro",
        description:
          "Informações de pagamento não encontradas. Por favor, volte e tente novamente.",
        variant: "destructive",
      });
      ...
      try {
        ...
        const formattedBillingData = {
          customer: {
            ...
            cpfCnpj: (billingData.customer?.cpfCnpj || "").replace(
              /[^0-9]/g,
              "",
            ),
          ...
          description:
            billingData.description || `Pedido ${orderData.id || "novo"}`,
        };
        ...
        if (
          !formattedBillingData.customer.name ||
          !formattedBillingData.customer.cpfCnpj
        ) {
        ...
        if (data.paymentId && orderData.id) {
          try {
            ...
            if (updateError) {
              console.error(
                "Error updating order with Asaas payment ID:",
                updateError,
              );
            } else {
              console.log(
                "Order updated with Asaas payment ID:",
                data.paymentId,
              );
              // Update the local order state with the payment ID
              setOrder((prev) =>
                prev ? { ...prev, asaasPaymentId: data.paymentId } : null,
              );
            }
          } catch (updateError) {
            console.error(
              "Exception updating order with Asaas payment ID:",
              updateError,
            );
          }
        }
        ...
        if (data && orderData) {
          try {
            await supabaseClientService.sendPixPaymentNotification(
              orderData,
              data,
            );
        ...
        console.log(
          "QR Code Image:",
          safePaymentData.qrCodeImage
            ? `Received (${safePaymentData.qrCodeImage.substring(0, 30)}...)`
            : "Not received",
        );
        console.log(
          "QR Code:",
          safePaymentData.qrCode
            ? `Received (${safePaymentData.qrCode.substring(0, 30)}...)`
            : "Not received",
        );
        console.log(
          "Copy Paste Key:",
          safePaymentData.copyPasteKey
            ? `Received (${safePaymentData.copyPasteKey.substring(0, 30)}...)`
            : "Not received",
        );
        ...
        const errorMessage = handleApiError(error, {
          defaultMessage:
            "Não foi possível gerar o pagamento PIX. Por favor, tente novamente.",
        });
