...
export const useWebhookActions = (
  setProcessingOrders: React.Dispatch<
  ...
  const simulatePaymentWebhook = async (
    ...
    try {
      ...
      console.log(
        `Using webhook endpoint: ${webhookEndpoint} for order ID: ${orderId}`,
      );
      console.log(
        `Is manual card: ${isManualCard}, Asaas Payment ID: ${asaasPaymentId || "None"}`,
      );
