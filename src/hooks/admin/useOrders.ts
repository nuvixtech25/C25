...
export function useOrders(
  initialPaymentMethod: PaymentMethod = "pix",
): UseOrdersReturn {
  const { orders, loading, fetchOrders, setOrders, setLoading } =
    useOrdersState();
  ...
  useEffect(() => {
    console.log(
      `[useOrders] Fetching orders with payment method: ${paymentMethod}, status: ${statusFilter}`,
    );
    fetchOrders({
  ...
  const ordersSummary = {
    ...
    totalValue: orders.reduce(
      (total, order) => total + Number(order.productPrice),
      0,
    ),
  };

  const ordersActions = useOrdersActions(orders, paymentMethod, fetchOrders);
