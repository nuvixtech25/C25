  ...
  const checkSmtpStatus = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setConfigured(false);
        setMessage(
          "Configuração SMTP incompleta. Configure as variáveis de ambiente SMTP_HOST, SMTP_USER e SMTP_PASS no Netlify.",
        );
      } else {
        setConfigured(false);
        setMessage(
          `Erro na configuração SMTP: ${data.message || "Erro desconhecido"}`,
        );
      }
    } catch (error) {
      setConfigured(false);
      setMessage(
        "Erro ao verificar configuração SMTP. Verifique as variáveis de ambiente.",
      );
