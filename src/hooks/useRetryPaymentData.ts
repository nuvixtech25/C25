...
export const RETRYABLE_STATUSES: PaymentStatus[] = [
  "FAILED",
  "DECLINED",
  "CANCELLED",
];
...
export const useRetryPaymentData = () => {
  ...
  const { hasWhatsappSupport, whatsappNumber } = useWhatsAppSupport(
    order?.productId,
  );
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      ...
      try {
        ...
        if (state?.order) {
          console.log(
            "[RetryPaymentData] Using order from state:",
            state.order,
          );
          ...
          if (!state.order.id || !state.order.productPrice) {
            console.error(
              "[RetryPaymentData] Order from state is missing required fields:",
              state.order,
            );
            setHasError(true);
            toast({
          ...
          if (
            state.order.status &&
            !RETRYABLE_STATUSES.includes(state.order.status as PaymentStatus)
          ) {
            console.warn(
              "[RetryPaymentData] Order status is not retryable:",
              state.order.status,
            );
          ...
          if (!orderId) {
            console.error(
              "[RetryPaymentData] No orderId found in URL parameters",
            );
            setHasError(true);
            toast({
              title: "Erro",
          ...
          if (!fetchedOrder.id || !fetchedOrder.productPrice) {
            console.error(
              "[RetryPaymentData] Fetched order is missing required fields:",
              fetchedOrder,
            );
            setHasError(true);
            toast({
          ...
          if (
            fetchedOrder.status &&
            !RETRYABLE_STATUSES.includes(fetchedOrder.status as PaymentStatus)
          ) {
            console.warn(
              "[RetryPaymentData] Order status is not retryable:",
              fetchedOrder.status,
  ...
  const checkRetryLimit = async (orderId: string) => {
    try {
      ...
      if (validatedOrderIds.current.has(orderId)) {
        console.log(
          "[RetryPaymentData] Skipping validation for already validated order:",
          orderId,
        );
        return;
      }
      ...
      console.log(
        "[RetryPaymentData] Checking retry limit for order:",
        orderId,
      );
