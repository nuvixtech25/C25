...
export const usePaymentPixelTracker = (
  ...
  useEffect(() => {
    if (order?.id && paymentData) {
      console.log(
        "[AUDIT][PixelTracker] Tracking payment initiated for order:",
        order.id,
      );
      // Track the start of the payment process
      trackInitiateCheckout(paymentData.value || order.productPrice);
    }
  ...
  useEffect(() => {
    if (order?.id && paymentStatus === "CONFIRMED") {
      console.log(
        "[AUDIT][PixelTracker] Tracking payment completed for order:",
        order.id,
      );
      // When payment is confirmed, track purchase event across all pixels
      trackPurchase(order.id, order.productPrice);
    }
