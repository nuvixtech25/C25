...
export const usePaymentNavigation = () => {
  ...
  const navigateToPayment = (
    ...
    console.log(
      "[usePaymentNavigation] WhatsApp do produto:",
    ...
    if (paymentMethod === "pix") {
      navigate("/payment", {
        state: {
          ...
          product: product
            ...
            : undefined,
          whatsapp_number:
            product?.whatsapp_number || currentOrder?.whatsapp_number,
        },
      });
    } else {
      ...
      if (targetPage === "/payment-failed") {
        console.log(
          `[usePaymentNavigation] Redirecting /payment-failed to /failed`,
        );
        targetPage = "/failed";
      } else {
        console.log(
          `[usePaymentNavigation] Navigating to configured redirect page: ${targetPage}`,
        );
      }
      ...
      const safeOrderData = currentOrder
        ? {
            ...
            customerCpfCnpj:
              currentOrder.customerCpfCnpj ||
              (customerData ? customerData.cpfCnpj : ""),
            customerPhone:
            ...
            whatsapp_number:
              currentOrder.whatsapp_number || product?.whatsapp_number,
          }
        : null;
  ...
  const navigateToFailure = (
    ...
    const safeOrderData = currentOrder
      ? {
          ...
          customerName:
            currentOrder.customerName ||
            (customerData ? customerData.name : "Cliente Anônimo"),
          customerEmail:
            currentOrder.customerEmail ||
            (customerData ? customerData.email : "anonimo@example.com"),
          productName: currentOrder.productName || "",
          productPrice: currentOrder.productPrice || 0,
    ...
    console.log(
      "[usePaymentNavigation] Navigating to failed page with order data:",
      orderDataToUse,
    );
