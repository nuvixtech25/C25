  ...
  useEffect(() => {
    const loadRedirectConfig = async () => {
      try {
        ...
        if (error) {
          console.error(
            "Erro ao carregar configuração de redirecionamento:",
            error,
          );
        ...
        if (data && data.manual_card_redirect_page) {
          console.log(
            "Configuração de redirecionamento carregada:",
            data.manual_card_redirect_page,
          );
          setRedirectPage(data.manual_card_redirect_page);
        }
      } catch (err) {
        console.error(
          "Falha ao carregar configuração de redirecionamento:",
          err,
        );
      }
    };
