...
export const trackInitiateCheckout = (
  ...
  console.log(
    `[PIXEL AUDIT] Tracking initiate checkout: Value=${value}, Initialized=${isInitialized}`,
  );

  if (process.env.NODE_ENV !== "production" && !isInitialized) {
    console.log(
      "[PIXEL AUDIT] Initiate checkout tracking skipped: development mode or pixels not initialized",
    );
    return;
  }
  ...
  if (typeof window !== "undefined" && window.googleAdsPixels) {
    window.googleAdsPixels.forEach((pixel) => {
      console.log(
        `[PIXEL AUDIT] Firing Google Ads begin_checkout for ID: ${pixel.googleAdsId}`,
      );
  ...
  if (typeof window !== "undefined" && window.facebookPixels) {
    window.facebookPixels.forEach((pixel) => {
      console.log(
        `[PIXEL AUDIT] Firing Facebook InitiateCheckout for ID: ${pixel.facebookPixelId}`,
      );
  ...
  if (typeof window !== "undefined" && window.tiktokPixelId) {
    console.log(
      `[PIXEL AUDIT] Firing TikTok InitiateCheckout for ID: ${window.tiktokPixelId}`,
    );
  ...
  if (typeof window !== "undefined" && window.taboolaAccountId) {
    console.log(
      `[PIXEL AUDIT] Taboola doesn't have InitiateCheckout event, skipping for ID: ${window.taboolaAccountId}`,
    );
  }
};
