...
export const usePreviewCustomization = (searchParams: URLSearchParams) => {
  const [customization, setCustomization] =
    useState<CheckoutCustomization>(defaultCustomization);
  const [demoProduct, setDemoProduct] = useState<Product>(getDemoProduct());

  useEffect(() => {
    // Parse customization from URL parameters
    const buttonColor =
      searchParams.get("buttonColor") || defaultCustomization.buttonColor;
    const buttonText =
      searchParams.get("buttonText") || defaultCustomization.buttonText;
    const headingColor =
      searchParams.get("headingColor") || defaultCustomization.headingColor;
    const bannerImageUrl =
      searchParams.get("bannerImageUrl") || defaultCustomization.bannerImageUrl;
    const topMessage =
      searchParams.get("topMessage") || defaultCustomization.topMessage;
    const countdownEndTime =
      searchParams.get("countdownEndTime") ||
      defaultCustomization.countdownEndTime;
    const isDigitalProduct = searchParams.get("isDigitalProduct") !== "false";
    const bannerColor =
      searchParams.get("bannerColor") || defaultCustomization.bannerColor;
