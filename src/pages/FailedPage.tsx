...
const FailedPage = () => {
    ...
    const loadOrderData = async () => {
      ...
      if (state?.order) {
        ...
        if (state.order.id && !state.order.id.startsWith("temp_")) {
          try {
            const attemptsCount =
              await supabaseClientService.getCardAttemptsCount(state.order.id);
      ...
      if (orderId) {
        try {
          ...
          if (fetchedOrder) {
            console.log(
              "[FailedPage] Successfully fetched order from database:",
              fetchedOrder,
            );
            ...
            try {
              const attemptsCount =
                await supabaseClientService.getCardAttemptsCount(orderId);
              console.log("[FailedPage] Card attempts count:", attemptsCount);
              setShouldShowRetryButton(attemptsCount < 3); // Hide retry button if max attempts reached
            } catch (err) {
              console.error("[FailedPage] Error checking card attempts:", err);
        ...
        console.log(
          "[FailedPage] No order found in location state and no orderId in URL",
        );
        handleMissingOrder();
      }
    };
  ...
  useEffect(() => {
    ...
    console.log(
      "[FailedPage] Should show retry button:",
