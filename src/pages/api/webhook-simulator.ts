  ...
  try {
    ...
    if (payload.event && payload.payment) {
      ...
      if (isManualCardPayment) {
        console.log(
          `Processing manual card webhook for order ${payload.orderId} with event ${payload.event}`,
        );
      } else {
        console.log(
          `Processing webhook for payment ${payload.payment.id} with event ${payload.event} and status ${payload.payment.status}`,
        );
      }
      ...
      if (!isManualCardPayment) {
        ...
        if (paymentsError) {
          console.log(
            "Note: Could not update asaas_payments table:",
