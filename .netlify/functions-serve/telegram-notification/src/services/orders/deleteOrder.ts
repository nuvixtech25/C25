...
export const deleteOrder = async (orderId: string) => {
  ...
  try {
    ...
    if (paymentError) {
      ...
      throw new Error(
        `Failed to check associated payment: ${paymentError.message}`,
      );
    }

    if (paymentData && paymentData.length > 0) {
      console.log(
        `Found associated payment for order ${orderId}, deleting it first`,
      );
      ...
      if (deletePaymentError) {
        ...
        throw new Error(
          `Failed to delete associated payment: ${deletePaymentError.message}`,
        );
      }
    }
    ...
    if (cardError) {
      ...
      throw new Error(
        `Failed to check associated card data: ${cardError.message}`,
      );
    }

    if (cardData && cardData.length > 0) {
      console.log(
        `Found associated card data for order ${orderId}, deleting it first`,
      );
      ...
      if (deleteCardError) {
        ...
        throw new Error(
          `Failed to delete associated card data: ${deleteCardError.message}`,
        );
      }
    }
