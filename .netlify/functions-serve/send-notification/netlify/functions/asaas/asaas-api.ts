...
export async function createAsaasCustomer(
  ...
  try {
    ...
    console.log(
      "Usando chave API:",
      apiKey ? `${apiKey.substring(0, 8)}...` : "Não definida",
    );
    ...
    if (!response.ok) {
      ...
      throw new AsaasApiError(
        `Erro ao criar cliente no Asaas: ${errorResponse.message || statusText}`,
        errorResponse,
      );
    }
...
export async function createAsaasPayment(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new AsaasApiError(
        `Erro ao criar pagamento PIX no Asaas: ${errorResponse.message || statusText}`,
        errorResponse,
      );
    }
...
export async function getAsaasPixQrCode(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new AsaasApiError(
        `Erro ao buscar QR Code PIX: ${errorResponse.message || statusText}`,
        errorResponse,
      );
    }
    ...
    console.log(
      `QR code received successfully. Encoded image length: ${data.encodedImage ? data.encodedImage.length : 0}`,
    );
